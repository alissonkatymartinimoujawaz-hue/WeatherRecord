...
const statusName=s=>({era5:'ERA5',era5_recent:'ERA5 récent · révisable',ifs_provisional:'IFS · provisoire',nasa:'NASA POWER',nasa_recent:'NASA POWER · récent, révisable',missing:'Données absentes'}[s]||s);
...
function renderDaily(){
 ...
 $('coverage').textContent=datefmt(date)+' · '+region.name;$('daily-status').textContent=statusName(row[5])+' · jours complets uniquement. Les données récentes peuvent être révisées ; aucune valeur du jour en cours.';
 const rs=data.rows.slice(Math.max(0,idx-Number($('window').value)+1),idx+1),point=(r,j)=>({x:Date.parse(r[0]+'T00:00:00Z'),y:r[j]});
 plot('daily-rain',[{name:'Pluie',color:'#007f87',bar:true,points:rs.map(r=>point(r,1))},{name:'Normale',color:'#223c52',dash:'5 4',points:rs.map(r=>({x:Date.parse(r[0]+'T00:00:00Z'),y:data.daily_normal[r[0].slice(5)]?.[0]??null}))}],{daily:true,rainBars:true,unit:'mm',axis:data.timezone==='LST'?'Date · temps solaire local':'Date · America/Sao_Paulo'});
 plot('daily-temp',[...[[2,'Tmean','#c76b20'],[3,'Tmin','#4279bd'],[4,'Tmax','#b74351']].map(([j,name,color])=>({name,color,points:rs.map(r=>point(r,j))})),{name:'Normale Tmean',color:'#223c52',dash:'5 4',points:rs.map(r=>({x:Date.parse(r[0]+'T00:00:00Z'),y:data.daily_normal[r[0].slice(5)]?.[1]??null}))}],{daily:true,unit:'°C',axis:data.timezone==='LST'?'Date · temps solaire local':'Date · America/Sao_Paulo'});
 $('daily-soil-article').hidden=!data.soil_daily;
 if(data.soil_daily)plot('daily-soil',[{name:'Humidité racinaire',color:'#268750',points:rs.map(r=>({x:Date.parse(r[0]+'T00:00:00Z'),y:data.soil_daily[r[0]]??null}))},{name:'Normale',color:'#223c52',dash:'5 4',points:rs.map(r=>({x:Date.parse(r[0]+'T00:00:00Z'),y:data.soil_daily_normal[r[0].slice(5)]??null}))}],{daily:true,unit:'fraction',axis:'Date · temps solaire local'});
...
function renderCompare(){
 ...
 $('compare-note').textContent=`${region.name} · ${ns?'NASA POWER':'ERA5 archivé'} · dernier jour : ${datefmt(src.last_date)}. Normale 1991–2020 propre à la source. Survole une courbe pour lire uniquement son année ; clique pour figer. Jours absents laissés vides.`;
 ...
 if(monthly&&act.includes(ly)&&currentDay<new Date(Date.UTC(currentYear,currentMonth,0)).getUTCDate()){
  ...
  $('compare-note').textContent=`Mois en cours : ${MONTHS[currentMonth-1]}, du 1 au ${currentDay} uniquement, pour toutes les années et la normale. Aucun total de mois complet n’est estimé. ${region.name} · ${ns?'NASA POWER':'ERA5 archivé'}. Survole une courbe pour lire son année.`;
 }
...
async function loadRegion(){
 ...
 document.querySelectorAll('.view').forEach(e=>e.hidden=true);$('freshness').textContent='Chargement des données…';
 ...
 try{
  const res=await fetch(folder+file+'.json',{cache:'no-cache'});if(!res.ok)throw Error('Les données ne sont pas disponibles pour cette sélection.');
  const payload=await res.json();if(version!==loadVersion)return;
  if(payload.region_id!==nextRegion.id)throw Error('Identifiant géographique incohérent.');
  ...
  $('freshness').innerHTML=`${ns?'NASA POWER · actualisation quotidienne':'ERA5 · archive, sans actualisation automatique'}<br>Dernier jour : ${datefmt(data.last_date)}<br>Collecte : ${new Date(data.updated_utc).toLocaleString('fr-FR',{timeZone:'Europe/London',dateStyle:'short',timeStyle:'short'})} · Londres${ns&&age>2?'<br>Actualisation en retard':''}`;
  $('brand-region').textContent=region.name+' · BRÉSIL';$('region-scope').textContent=region.scope;
  ...
  $('selection-note').textContent=region.selection_note||'Quatre points à poids égaux. Boa Esperança et Guapé se trouvent dans la même maille MERRA-2, qui pèse donc 50 % de cet indice.';
  ...
  switchView(view);
 }catch(e){if(version!==loadVersion)return;$('error').hidden=false;$('error').textContent=e.message;$('freshness').textContent='Données indisponibles';}
}
async function init(){try{
 if(!window.d3)throw Error('La bibliothèque graphique n’a pas pu être chargée. Actualise la page.');
 const res=await fetch('data/regions.json',{cache:'no-cache'});if(!res.ok)throw Error('Régions indisponibles');regions=await res.json();
